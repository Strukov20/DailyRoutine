...
| Components              | Jest + React Native Testing Library                                                           | Reusable UI primitives (`src/components/ui/**`) and feature components (`src/components/tasks/**`, `src/components/calendar/**`, `src/components/sync-issues/**`) rendered and interacted with — accessibility state (`accessibilityState.checked`, not color alone), double-submit/duplicate-tap prevention, the Day Calendar's Personal/Family modes, member filters, chronological rendering, Busy-block non-navigability, conflict-warning content (no private title ever rendered), and (Phase 9 completion pass) the Sync Issues list/comparison/resolution screens — every issue type and status, field-level comparison only for stale-write conflicts, Retry/Keep/Apply/Discard confirmation flows, offline-disabled actions, and a dedicated "no raw Postgres error ever rendered" assertion; and (Phase 10) the family settings danger zone (owner sees Delete not Leave and vice versa, both `Alert.alert`-gated), the member-detail "Make family owner" flow, and the profile screen's typed-confirmation account-deletion dialog — see "Alert.alert needs a spy, not a fireEvent.press" below for the shared native-confirmation test pattern all three use                                                                                                                                                                                                                                                                                                                                                                                                            | `src/components/ui/EmptyState.test.tsx`, `src/components/tasks/{TaskRow,QuickAddInput}.test.tsx`, `src/components/calendar/{ResponsibilityRow,EventEditorForm,CalendarScreen}.test.tsx` (Phase 7 follow-up — see below for why the Calendar Day view's own test lives in `src/components/calendar/` rather than co-located with `app/(app)/calendar.tsx`), `src/components/sync-issues/{SyncIssueCard,SyncIssuesScreen,SyncIssueDetailScreen}.test.tsx` (same outside-`app/` convention, Phase 9 completion pass), `src/components/family/{FamilyScreen,FamilyMemberDetailScreen}.test.tsx` and `src/components/profile/ProfileScreen.test.tsx` (same outside-`app/` convention, Phase 10). `EventEditorForm.test.tsx` is scoped to what's reliably observable under RNTL, per the same `react-native-paper` `<Menu>` limitation Phase 5 documented — trigger buttons, disabled state, and validation/submission logic are tested; opened-menu content is not. |
| Repositories / services | Jest, with `@/lib/supabase/client` mocked at the module boundary                              | Every `<feature>Service.ts`'s row-mapping and SQLSTATE → typed-error-code normalization (see ARCHITECTURE.md, "Layering"); `notificationService.ts`'s simulator/emulator short-circuit, permission-denied/registration-failure typed errors, and logout-time token deactivation (never throws, even if the underlying call does)                                                                                                                                                                                                                                                                                                                                                                                                                                | `src/lib/family/familyService.test.ts` (Phase 10: also the three new ownership/deletion RPC wrappers), `src/lib/tasks/taskService.test.ts`, `src/lib/categories/categoryService.test.ts`, `src/lib/notifications/notificationService.test.ts`, `src/lib/profile/profileService.test.ts` (Phase 10, new)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| RLS / privacy           | pgTAP via `supabase test db`, against a real local Postgres instance with RLS enabled         | Non-owner cannot read a private row's sensitive columns via the base table or the sanitized view; the required personas (owner, adult member, outsider, different-family adult, unlinked child, anonymous) each get the correct allow/deny outcome per table; the "Event ≠ Responsibility" worked example (renaming an event leaves its responsibilities untouched, `due_at` derives from the event and never drifts); append-only enforcement on `task_assignments`/`responsibility_assignments`; every family/invitation/child-profile/personal-task/shared-task/notification/calendar RPC's permission and validity checks, and that `anon` truly has no `EXECUTE` on any of them (see DECISIONS.md, "Phase 3," for why this needed a dedicated real-instance check rather than trusting the migration's own `revoke` statement) — for notifications specifically, that the entire `notifications` schema (not just its RLS) is inaccessible to `authenticated`/`anon`; for calendar specifically, the responsibility assignment state machine, Busy-block privacy for private family-linked events, and `has_member_schedule_conflict`'s half-open interval semantics; and (Phase 6.1) a durable, schema-driven regression guard asserting that the anon/PUBLIC-EXECUTE invariant holds for *every* function in `public`/`notifications`, present or future, not just the ones a migration author remembered to test individually | **Implemented and verified** — `supabase/tests/*.sql` (18 files, `Files=18, Tests=578` as of Phase 10 — see `010`–`180`: `170_release_safety_test.sql` (Phase 10, 40 assertions — the four family-lifecycle RPCs: authorization, the atomic ownership-transfer write ordering, never-orphan-an-owner, existence-oracle-free `42501`s for an arbitrary target member id, append-only audit-trail correctness, idempotent double-calls, anon denial, and the internal `_remove_or_leave_family_member` helper's own unreachability) and `180_task_category_guard_test.sql` (Phase 10, 8 assertions — cross-family category rejection on both `create_personal_task`/`update_personal_task`, system-category and own-family acceptance); `110_notification_outbox_test.sql` (Phase 6, 30 assertions), `120_family_calendar_test.sql` (Phase 7, 92 assertions — 88 original + 4 added in the Phase 7 follow-up audit closing a gap where the `declined`/`taken` notification event types had no pgTAP assertion of their own), `130_security_regression_test.sql` (Phase 6.1, 6 assertions), `140_recurring_tasks_reminders_test.sql` (Phase 8), `150_realtime_offline_conflicts_test.sql` (Phase 9, 47 assertions — Realtime topic RLS for profile/family/outsider/removed-member/malformed/anon personas, no client INSERT on `realtime.messages`, generic-payload-content checks, offline-operation idempotency and cross-user isolation, stale-write conflicts for both `update_personal_task`/`schedule_personal_task`, all 8 conflict-detection categories, conflict-id stability, and secret-marker sweeps over both broadcast payloads and conflict rows), and `160_sync_issues_resolution_test.sql` (final security/concurrency pass, 22 assertions — proves, for every one of the six affected RPCs, that a random never-existed UUID and another profile's real task/occurrence UUID raise byte-for-byte identical SQLSTATE+message via a `pg_temp` capture helper, not just "both happen to be 42501"; also covers both occurrence RPCs' idempotent-replay guarantee, `update_personal_task` never touching a field outside the ones actually passed, and a second concurrent write after a resolved conflict correctly raising `40001` again). All 578 assertions pass against a real local instance (`supabase db reset && supabase test db`), plus real curl-driven multi-user flows for Family Space, personal tasks, shared tasks, notifications, and the calendar (owner/member/outsider, real `auth.users` accounts) — see `scripts/e2e-backend.sh`/`e2e:backend`, `scripts/e2e-notifications.sh`/`e2e:notifications`, `scripts/e2e-calendar.sh`/`e2e:calendar`, and docs/DECISIONS.md for the Phase 3–9 and 6.1 final reports. Phase 9's own real local Realtime WebSocket integration test and offline-integration test suite are implemented and passing — `scripts/e2e-realtime.mjs`/`npm run e2e:realtime` (5 real WebSocket personas, 28 assertions) and `src/lib/offline/__e2e__/offlineQueue.e2e.test.ts`/`npm run e2e:offline` (the production offline queue against real local RPCs, via `jest.e2e.config.js`, extended with a real end-to-end proof of the "stale review" concurrency window: stale conflict → server-version fetch/compare (records `reviewedVersion`) → Keep server version (never replays again) → a second conflict → review → Apply my change (only the original patch's fields change) → a third conflict → review → a further real concurrent write lands → Apply refuses to mutate (`'stale_review'`, comparison refreshed in place) → an explicit second Apply, with nothing else having changed, succeeds → a repeat Apply is a safe no-op → zero residue; the tighter fetch-to-write race window is proven separately at the unit level in `syncIssueResolution.test.ts`, by mocking the RPC call itself to reject after the version check has already passed — real concurrent timing at that granularity can't be reproduced from sequential test code), each run twice consecutively with zero residue confirmed directly against the database. See `knowledge/wiki/engineering/realtime-sync-and-offline.md` for current status. |
...
- **`Alert.alert()`-based confirmations (Phase 10) need a spy, not a `fireEvent.press`** — React
  Native's native `Alert.alert()` never renders into the RNTL component tree, so its buttons
  can't be found or pressed through the normal query/fireEvent API. The pattern, established in
  `FamilyScreen.test.tsx` and reused in `FamilyMemberDetailScreen.test.tsx`/
  `ProfileScreen.test.tsx`'s dialog variant: `jest.spyOn(Alert, 'alert')`, read the `buttons`
  array off the spy's last call (`alertSpy.mock.calls.at(-1)?.[2]`), and invoke the target
  button's `onPress` inside `await act(async () => { await button.onPress?.(); })`. A Cancel
  button with no `onPress` (native dismiss-by-default, this codebase's existing convention)
  can't be "pressed" this way — assert its presence and `style: 'cancel'` instead, and that no
  destructive call fired.
- **The same "unawaited act() corrupts a later test" failure mode above (Phase 9) recurred via
  an unawaited `fireEvent.press` specifically (Phase 10)** — this installed RNTL version's own
  requirement (see the top of this section) applies to `fireEvent.press` too, not just bare
  `act()`. Writing `FamilyMemberDetailScreen.test.tsx` reproduced it exactly: the offending
  test itself passed; the *next* test's `render()` silently returned an empty tree, preceded by
  an "overlapping act() calls" warning easy to misread as unrelated noise. Fixed by awaiting
  every `fireEvent.press` call. Recorded again here, distinctly, because the trigger this time
  (`fireEvent.press`, not `act()` directly) wasn't obviously the same bug at first glance.

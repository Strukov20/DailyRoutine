...
| Components              | Jest + React Native Testing Library                                                           | Reusable UI primitives (`src/components/ui/**`) and feature components (`src/components/tasks/**`, `src/components/calendar/**`, `src/components/sync-issues/**`) rendered and interacted with — accessibility state (`accessibilityState.checked`, not color alone), double-submit/duplicate-tap prevention, the Day Calendar's Personal/Family modes, member filters, chronological rendering, Busy-block non-navigability, conflict-warning content (no private title ever rendered), and (Phase 9 completion pass) the Sync Issues list/comparison/resolution screens — every issue type and status, field-level comparison only for stale-write conflicts, Retry/Keep/Apply/Discard confirmation flows, offline-disabled actions, and a dedicated "no raw Postgres error ever rendered" assertion                                                                                                                                                                                                                                                                                                                                                                                                            | `src/components/ui/EmptyState.test.tsx`, `src/components/tasks/{TaskRow,QuickAddInput}.test.tsx`, `src/components/calendar/{ResponsibilityRow,EventEditorForm,CalendarScreen}.test.tsx` (Phase 7 follow-up — see below for why the Calendar Day view's own test lives in `src/components/calendar/` rather than co-located with `app/(app)/calendar.tsx`), `src/components/sync-issues/{SyncIssueCard,SyncIssuesScreen,SyncIssueDetailScreen}.test.tsx` (same outside-`app/` convention, Phase 9 completion pass). `EventEditorForm.test.tsx` is scoped to what's reliably observable under RNTL, per the same `react-native-paper` `<Menu>` limitation Phase 5 documented — trigger buttons, disabled state, and validation/submission logic are tested; opened-menu content is not. |

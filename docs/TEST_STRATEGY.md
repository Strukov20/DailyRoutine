...
| Components              | Jest + React Native Testing Library                                                           | Reusable UI primitives (`src/components/ui/**`) and feature components (`src/components/tasks/**`) rendered and interacted with — accessibility state (`accessibilityState.checked`, not color alone), double-submit/duplicate-tap prevention                                                                                                                                                                                                                                                                                                                                                                                                            | `src/components/ui/EmptyState.test.tsx`, `src/components/tasks/{TaskRow,QuickAddInput}.test.tsx`. **Not yet covered**: `src/components/calendar/{EventEditorForm,ResponsibilityRow}.tsx` and `app/(app)/calendar.tsx` (Phase 7) — deferred given this phase's already-large scope and the documented RNTL/`react-native-paper` `<Menu>`/list-virtualization limitations from Phase 5 that both new components would likely hit (multiple `Menu`-based pickers in `EventEditorForm`); the real backend integration (`scripts/e2e-calendar.sh`) and domain-logic unit tests are this phase's UI-adjacent coverage instead. |

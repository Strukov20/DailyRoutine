 ...
 *   npm run db:types
 *
 * (wraps `supabase gen types typescript --local`). Note: CHECK-constrained
 * columns (priority, visibility, role, status, etc.) come back as `string`
 * here — Postgres CHECK constraints aren't reflected in generated types.
 * Narrow them to literal unions in domain mappers (see
 * src/domain/profile/mappers.ts for the pattern), not by hand-editing this
 * file.
 */
